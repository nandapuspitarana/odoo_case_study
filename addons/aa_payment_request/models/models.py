...
class PaymentRequest(models.Model):
    ...
    # FIELD SETTLEMENT
    advance_payment_id = fields.Many2one('payment.request', 'AAP', readonly=True, states={'draft': [('readonly', False)]}, domain=[
                                         ('type', '=', 'aap'), ('state', '=', 'paid'), ('advance_payment_id', '=', False)], track_visibility='onchange')
    amount_aap = fields.Monetary(
        'Amount', related='advance_payment_id.amount_total', track_visibility='onchange')
    settlement_line = fields.One2many('settlement.line', 'payment_request_id',
                                      'Payment Lines', readonly=True, states={'draft': [('readonly', False)]})
    settlement_total = fields.Monetary(
        'Total', store=True, compute='_amount_settlement', track_visibility='onchange')
    amount_diff = fields.Monetary(
        'Difference', store=True, compute='_amount_diff', track_visibility='onchange')

    count_journal = fields.Integer(
        compute='compute_journal', string='Journal Entries')
    account_move_line = fields.One2many(
        'account.move', 'payment_request_id', 'Account Move Lines', readonly=True)
    ...
    def payment_open(self):
        for o in self:
            if o.type == 'as' and o.amount_diff < 0:
                raise UserError(
                    ("Field Difference tidak boleh minus (pengeluaran biaya lebih besar dari uang muka) ! "))
    ...
    def payment_done(self):
        for o in self:
            if o.settlement_line:
                ...
                move_line_id = obj_move_line.search(
                    [('payment_request_line_id', '=', self.advance_payment_id.payment_line.id), ('balance', '>', 0)])
                move_lines.append((0, 0, {
...
class AccountMoveLine(models.Model):
    ...
    payment_request_line_id = fields.Many2one(related='statement_line_id.payment_request_line_id', depends=[
                                              'statement_line_id'], store=True, string='APR')
